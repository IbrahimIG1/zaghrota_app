...
class ShabkaSongsRepo {
   HiveStorage hiive = HiveStorage();

  Future<Either<ErrorModel,void>> addSongs(SongModel songData)async {
  try {
    await hiive.addValue<SongModel>(boxName: BoxesNames.songsShabka, value: songData);
    return right(null);
  }
...
Either<ErrorModel,List<SongModel>> getSongs(){
  try {
    List<SongModel> data = hiive.getBoxValues<SongModel>(boxName: BoxesNames.songsShabka) as List<SongModel>;
    return right(data);
  }
    on HiveError catch (e){
    return left(ErrorModel(errormsg: "The error from hive is $e"));
...
Future<Either<ErrorModel,void>> updateOrderValue({required int index,required dynamic value})async{
  try {
    List data = hiive.getBoxValues<SongModel>(boxName: BoxesNames.songsShabka);
    if(data.isNotEmpty){
      
    await hiive.updateItem<SongModel>(boxName: BoxesNames.songsShabka, index: index, value:value);
    return right(null);
    }
    else{
      return right(null);
...
Future<Either<ErrorModel,Null>> deleteValue({required int index})async{
  try {
    // List data = hiive.getBoxValues(boxName: BoxesNames.songsShabka);
    
    await hiive.deleteItem<SongModel>(boxName: BoxesNames.songsShabka, index: index);
    return right(null);
...
Future<Either<ErrorModel, void>> updataSongData({required int index,required SongModel song})async{
  try{
   await hiive.updateItem<SongModel>(boxName: BoxesNames.songsShabka, index: index, value: song);
    return right(null);
  }
