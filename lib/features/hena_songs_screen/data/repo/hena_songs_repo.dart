...
class HenaSongsRepo {
  ...
  try {
    await hiive.addValue<SongModel>(boxName: BoxesNames.songshena, value: songData);
    return right(null);
  }
...
Future<Either<ErrorModel,void>> updateOrderValue({required int index,required dynamic value})async{
  try {
    List data = hiive.getBoxValues<SongModel>(boxName: BoxesNames.songshena);
    if(data.isNotEmpty){
      
    await hiive.updateItem<SongModel>(boxName: BoxesNames.songshena, index: index, value:value);
    return right(null);
    }
    else{
      return right(null);
...
Future<Either<ErrorModel,Null>> deleteValue({required int index})async{
  try {
    // List data = hiive.getBoxValues(boxName: BoxesNames.songshena);
    
    await hiive.deleteItem<SongModel>(boxName: BoxesNames.songshena, index: index);
    return right(null);
