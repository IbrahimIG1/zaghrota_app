...
class ArosaDevicesMafroshaatScreen extends StatelessWidget {
  const ArosaDevicesMafroshaatScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      floatingActionButton: BlocConsumer<DevicesMafroshaatScreenCubit, DevicesMafroshaatScreenState>(
                listener: (context, state) {
      
                },
                builder: (context, state) {
      var cubit = DevicesMafroshaatScreenCubit.get(context);
      ...
      body: SingleChildScrollView(
        child: BlocConsumer<DevicesMafroshaatScreenCubit, DevicesMafroshaatScreenState>(
          listener: (context, state) {
            
          },
          builder: (context, state) {
            if(state is DevicesMafroshaatScreenSuccess){
            var cubit = DevicesMafroshaatScreenCubit.get(context);
            ...
            return CustomListView(
              ...
              checkOnChanged: (p0, p1) {
                  state.mafroshaatDevices[p1].checked = p0!;
                                cubit.updateCheck(
                                    index: p1,
                                    model: DevicesModel(
                                        deviceName: state.mafroshaatDevices[p1].deviceName,
                                        number: state.mafroshaatDevices[p1].number,
                                        checked: state.mafroshaatDevices[p1].checked));
              },
              deleteOnPressed: (index) {
                deleteDialog(context, cubit, index);
                // cubit.deleteItem(index: index);
              },
              data: state.mafroshaatDevices,
            );}
          else if (state is DevicesMafroshaatScreenFailure){
  ...
  Future<void> updateDialog(BuildContext context, String? newval, DevicesMafroshaatScreenSuccess state, int index, DevicesMafroshaatScreenCubit cubit) {
                      ...
                      onChanged: (p0) {
                        ...
                        const VerticalSizedBox(height: 10),
                           ElevatedButton(
                            ...
                            onPressed: ()  {
                             if(newval!=null){
                              state.mafroshaatDevices[index].number = newval!;
                              cubit.updateCheck(
                                  index: index,
                                  model: DevicesModel(
                                      deviceName: state.mafroshaatDevices[index].deviceName,
                                      number: state.mafroshaatDevices[index].number,
                                      checked: state.mafroshaatDevices[index].checked));
   ...
   void deleteDialog(BuildContext context, DevicesMafroshaatScreenCubit cubit, int index) {
