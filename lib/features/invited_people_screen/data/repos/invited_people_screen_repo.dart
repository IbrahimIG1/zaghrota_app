...
class InvitedPeopleScreenRepo {
  ...
  try {
    await hiive.addValue(boxName: BoxesNames.invitedPeoples, value: inviteData);
    return right(null);
  }
...
Either<ErrorModel,List<InvitedModel>> getInvitedPeople(){
  try {
    List<InvitedModel> data = hiive.getBoxValues(boxName: BoxesNames.invitedPeoples) as List<InvitedModel>;
    return right(data);
  }
    on HiveError catch (e){
    return left(ErrorModel(errormsg: "The error from hive is $e"));
...
Future<Either<ErrorModel,void>> updateCheckedValue({required int index,required dynamic value})async{
  try {
    ...
    if(data.isNotEmpty){
    await hiive.updateItem(boxName: BoxesNames.invitedPeoples, index: index, value:value);
    return right(null);
    }
    else{
      return right(null);
...
Future<Either<ErrorModel,Null>> deleteValue({required int index})async{
  try {
    ...
    await hiive.deleteItem(boxName: BoxesNames.invitedPeoples, index: index);
    return right(null);
