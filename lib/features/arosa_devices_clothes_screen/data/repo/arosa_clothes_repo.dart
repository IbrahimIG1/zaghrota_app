...
class ArosaClothesRepo {
  HiveStorage hiive = HiveStorage();
Either<ErrorModel,List<bool>> getData (){
  try{
  List<bool> data = hiive.getBoxValues<bool>(boxName: BoxesNames.clothesChecks) as List<bool>;
  return right(data);
  }
...
Future<Either<ErrorModel,void>> addData({required bool value})async{
  ...
  if (data.length<16){
    try{
      await hiive.addValue<bool>(boxName: BoxesNames.clothesChecks, value:value );
      return right(null);
    }
       on HiveError catch (e){
    return left(ErrorModel(errormsg: "The error from hive is $e"));
...
Future<Either<ErrorModel,void>> updateCheckedValue({required int index,required bool value})async{
  try {
    List data = hiive.getBoxValues<bool>(boxName: BoxesNames.clothesChecks);
    if(data.isNotEmpty){
    await hiive.updateItem<bool>(boxName: BoxesNames.clothesChecks, index: index, value:value);
    return right(null);
    }
    else{
      return right(null);
