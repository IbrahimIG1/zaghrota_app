...
class ArosaDevicesR2sScreen extends StatelessWidget {
  const ArosaDevicesR2sScreen({super.key});
  ...
  Widget build(BuildContext context) {
     return SafeArea(
      ...
      floatingActionButton: BlocConsumer<R2sClothesCubit, R2sClothesState>(
                listener: (context, state) {
      
                },
                builder: (context, state) {
      var cubit = R2sClothesCubit.get(context);
      ...
      body: SingleChildScrollView(
        child: BlocConsumer<R2sClothesCubit, R2sClothesState>(
          listener: (context, state) {
            
          },
          builder: (context, state) {
            if(state is R2sClothesSuccess){
            var cubit = R2sClothesCubit.get(context);
          ...
          else if (state is R2sClothesFailure){
  ...
  Future<void> updateDialog(BuildContext context, String? newval, R2sClothesSuccess state, int index, R2sClothesCubit cubit) {
   ...
   void deleteDialog(BuildContext context, R2sClothesCubit cubit, int index) {
