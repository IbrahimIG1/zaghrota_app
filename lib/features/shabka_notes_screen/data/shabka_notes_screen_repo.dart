...
class ShabkaNotesScreenRepo {
  ...
  Future<Either<ErrorModel,void>> addData({required NoteModel note})async{
    try{
      ...
      hiive.getBoxValues<NoteModel>(boxName:BoxesNames.shabkaNotesBox) as List<NoteModel>;
 ...
 Future<Either<ErrorModel,void>> deleteData({required int index})async{
  ...
  try{
    await hiive.deleteItem<NoteModel>(boxName: BoxesNames.shabkaNotesBox, index: index);
    return right(null);
  }
    on HiveError catch (e){
    return left(ErrorModel(errormsg: "The error from hive is $e"));
