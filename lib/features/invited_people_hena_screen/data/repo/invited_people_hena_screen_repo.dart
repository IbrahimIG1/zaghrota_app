...
class InvitedPeopleHenaScreenRepo {
   HiveStorage hiive = HiveStorage();

  Future<Either<ErrorModel,void>> addInvitedPeople(InvitedModel inviteData)async {
  try {
    await hiive.addValue<InvitedModel>(boxName: BoxesNames.invitedPeopleHena, value: inviteData);
    return right(null);
  }
...
Either<ErrorModel,List<InvitedModel>> getInvitedPeople(){
  try {
    List<InvitedModel> data = hiive.getBoxValues<InvitedModel>(boxName: BoxesNames.invitedPeopleHena) as List<InvitedModel>;
    return right(data);
  }
    on HiveError catch (e){
    return left(ErrorModel(errormsg: "The error from hive is $e"));
...
Future<Either<ErrorModel,void>> updateCheckedValue({required int index,required dynamic value})async{
  try {
    List data = hiive.getBoxValues<InvitedModel>(boxName: BoxesNames.invitedPeopleHena);
    if(data.isNotEmpty){
    await hiive.updateItem<InvitedModel>(boxName: BoxesNames.invitedPeopleHena, index: index, value:value);
    return right(null);
    }
    else{
      return right(null);
...
Future<Either<ErrorModel,Null>> deleteValue({required int index})async{
  try {
    // List data = hiive.getBoxValues(boxName: BoxesNames.invitedPeopleHena);
    
    await hiive.deleteItem<InvitedModel>(boxName: BoxesNames.invitedPeopleHena, index: index);
    return right(null);
