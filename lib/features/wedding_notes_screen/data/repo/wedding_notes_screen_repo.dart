...
class WeddingNotesScreenRepo {
  ...
  hiive.getBoxValues<NoteModel>(boxName:BoxesNames.weddingNotesBox) as List<NoteModel>;
 ...
 Future<Either<ErrorModel,void>> deleteData({required int index})async{
  ...
  try{
    await hiive.deleteItem<NoteModel>(boxName: BoxesNames.weddingNotesBox, index: index);
    return right(null);
  }
    on HiveError catch (e){
    return left(ErrorModel(errormsg: "The error from hive is $e"));
